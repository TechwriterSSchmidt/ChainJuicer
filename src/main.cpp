...
void handleRoot() {
    ...
    footer.replace("%LED_DIM%", String(map(oiler.ledBrightnessDim, 0, 255, 0, 100)));
    footer.replace("%LED_HIGH%", String(map(oiler.ledBrightnessHigh, 0, 255, 0, 100)));
    ...
    footer.replace("%NIGHT_BRI%", String(map(oiler.nightBrightness, 0, 255, 0, 100)));
...
void handleSave() {
    ...
    if(server.hasArg("led_dim")) {
        ...
        oiler.ledBrightnessDim = map(val, 0, 100, 0, 255);
    }
    if(server.hasArg("led_high")) {
        int val = server.arg("led_high").toInt();
        if (val < 0) val = 0; if (val > 100) val = 100;
        oiler.ledBrightnessHigh = map(val, 0, 100, 0, 255);
    }
    ...
    if(server.hasArg("night_bri")) {
        ...
        if (val < 0) val = 0; if (val > 100) val = 100;
        oiler.nightBrightness = map(val, 0, 100, 0, 255);
    }
...
void loop() {
    ...
    if (oiler.isButtonPressed()) {
        if (currentSpeed < 7.0) { // threshold to 7 km/h to handle GPS drift
            if (!wifiButtonHeld) {

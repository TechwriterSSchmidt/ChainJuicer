...
void handleRoot() {
    ...
    for(int i=0; i<NUM_RANGES; i++) {
        ...
        float pct = 0.0;
        if (oiler.totalTimeSeconds > 0) {
             pct = (oiler.rangeTimeSeconds[i] / oiler.totalTimeSeconds) * 100.0;
        }
        ...
        html += "</td><td style='text-align:center;color:#555'>" + String(oiler.rangeOilingCounts[i]);
        html += "</td><td><input type='number' name='p" + String(i) + "' value='" + String(r->pulses) + "'></td></tr>";
    }
    
    // Add Reset Link below the table
    html += "</table><div style='text-align:right;margin-bottom:10px'><a href='/reset_time_stats' style='color:orange;text-decoration:none;font-size:0.8em'>[Reset Profile]</a></div>";
